...
public class DummyMarketServiceImpl implements IMarketService {
	...
	public DailyMarketActivityDTO getLastDayMarketActivity(String string) {
		...
		LocalDateTime localDateTime = LocalDateTime.parse("2015-04-10 17:30", formatter);
		return new DailyMarketActivityDTO("DAX 30","GDAXI", map, Date.from(localDateTime.toInstant(ZoneOffset.UTC)));
	}
