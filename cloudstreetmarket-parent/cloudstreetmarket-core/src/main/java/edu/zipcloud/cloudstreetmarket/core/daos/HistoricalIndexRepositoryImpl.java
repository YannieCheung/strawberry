...
import edu.zipcloud.util.DataUtil;
...
public class HistoricalIndexRepositoryImpl implements HistoricalIndexRepository {
	...
	public Iterable<HistoricalIndex> findIntraDay(String code, Date of) {
		TypedQuery<HistoricalIndex> sqlQuery = em.createQuery("from HistoricalIndex h where h.index.code = ? and h.fromDate >= ? and h.toDate <= ? "
				+ "ORDER BY h.toDate asc", HistoricalIndex.class);
		sqlQuery.setParameter(1, code);
		sqlQuery.setParameter(2, DataUtil.getStartOfDay(of));
		sqlQuery.setParameter(3, DataUtil.getEndOfDay(of));
	...
	@Override
	public Iterable<HistoricalIndex> findHistoric(String code, MarketCode market, Date fromDate, Date toDate,
