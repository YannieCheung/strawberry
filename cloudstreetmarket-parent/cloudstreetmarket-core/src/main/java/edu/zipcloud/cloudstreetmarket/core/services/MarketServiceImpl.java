...
public class MarketServiceImpl implements IMarketService {
	...
	public Page<IndexOverviewDTO> getLastDayIndicesOverview(MarketCode market, Pageable pageable) {
		...
		List<IndexOverviewDTO> results = new ArrayList<>();
		indices.forEach(
			index -> results.add(IndexOverviewDTO.build(index))
		);
		return new PageImpl<>(results, pageable, indices.getTotalElements());
	}
	...
	public Page<IndexOverviewDTO> getLastDayIndicesOverview(Pageable pageable) {
		...
		List<IndexOverviewDTO> result = new ArrayList<>();
